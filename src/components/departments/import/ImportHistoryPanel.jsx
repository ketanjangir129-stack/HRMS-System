...
import { getImportHistory } from "../../../services/departmentImportService";
...
| Import History
|--------------------------------------------------------------------------
| Every department import this company has run, newest first.
|
| Metadata only, which is all the history node stores: who ran what file, and
| what it created. The departments themselves are not here and never were -
| they are in the departments tree, which is the point of importing them.
|
    ...
    <div className="ui-card overflow-hidden">
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="ui-card-subtitle">
              The last {runs.length || ""} department imports for this company.
            </p>
      ...
      {!loading && !error && runs.length === 0 && (
        <EmptyState
          ...
          message="Departments you import will be listed here with who imported them and what happened."
        />
      )}
          ...
          <table className="w-full min-w-[52rem] border-collapse text-left text-sm">
            ...
            <thead>
              <tr className="border-b border-line bg-surface-muted text-xs uppercase tracking-wide text-ink-subtle">
                ...
                <th scope="col" className="px-4 py-3 text-center font-semibold">Departments</th>
                <th scope="col" className="px-4 py-3 text-center font-semibold">Designations</th>
              ...
              {runs.map((run) => (
                ...
                <tr key={run.importId} className="transition-colors hover:bg-surface-muted">
                  ...
                  <td className="px-4 py-4 text-center font-semibold text-emerald-600">
                    {(run.departmentsCreated || 0).toLocaleString()}
                  </td>

                  <td className="px-4 py-4 text-center font-semibold text-brand">
                    {(run.designationsCreated || 0).toLocaleString()}
                  </td>

                  <td
                    className={`px-4 py-4 text-center font-semibold ${
                      run.failedNodes ? "text-red-600" : "text-ink-faint"
                    }`}
                  >
                    {(run.failedNodes || 0).toLocaleString()}
                  </td>
